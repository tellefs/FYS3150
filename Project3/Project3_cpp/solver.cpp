...
void Solver::forwardEulerMethod(mat pos, mat vel, double force, int N, double h){
    ...
    //the matrix pos should contain 3 N-long arrays representing x,y and z position
    //same with matrix vel, only for the velocity
    // force is the value of the force which needs to be multiplied with the position
    //N is array length and h is the steplength
