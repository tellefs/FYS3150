...
In this project we solve the Schrödinger equation for two electrons in a 3D harmonic oscillator potential. We solve with and without electron repulsion, and compare the results. When comparing these results we saw that the factor $\omega_R$, which reflected the strength of the oscillator potential, made the wave function for the interacting and the non-interacting case differ more than the Coulomb-potential which were added to simulate the electron repulsion. \\ 
To accomplish this we apply a general method of discretizing the domain and reducing the problem to an eigenvalue equation. We thereafter apply Jacobi's rotation algorithm to obtain the eigenvalues of the matrix. This way of solving the eigenvalue-problem works in our case, but it is not as effective as using the Armadillo-library functions. We also apply the principles of unit testing by testing the algorithm for some simple problems with known solutions.\\\\
All code for this project can be retrieved at this git-hub address :

\begin{verbatim}
https://github.com/tellefs/FYS3150/tree/master/project2
\end{verbatim}
...
In this project we implemented the Jacobi-algorithm to solve our eigenvalue problem. We saw that this might not have been the most effective way to solve this, but it worked for our purpose. \\
We saw that the Coulomb-potential added a small change in the wave-function for the interacting and the non-interacting case, but the strength of the oscillator potential mad the difference larger. 
